...
final class TaskMybatisWriteMapper {
    ...
    TaskEntity toTaskEntity(TaskRecordAssembler.TaskWriteModel model) {
        ...
        entity.setTaskType(model.taskType());
    ...
    /**
     * 处理转为请求日志实体。
     * @param fallbackTaskId 兜底任务标识
     * @param requestLog 请求日志值
     * @return 处理结果
     */
    RequestLogEntity toRequestLogEntity(String fallbackTaskId, Map<String, Object> requestLog) {
        RequestLogEntity entity = new RequestLogEntity();
        String taskId = firstNonBlank(
            TaskMybatisValueSupport.stringValue(requestLog.get("taskId")),
            fallbackTaskId
        );
        String workflowId = TaskMybatisValueSupport.stringValue(requestLog.get("workflowId"));
        String ownerRefId = firstNonBlank(
            TaskMybatisValueSupport.stringValue(requestLog.get("ownerRefId")),
            taskId,
            workflowId
        );
        boolean success = TaskMybatisValueSupport.boolValue(requestLog.get("success"));
        entity.setRequestLogId(firstNonBlank(
            TaskMybatisValueSupport.stringValue(requestLog.get("requestLogId")),
            TaskMybatisValueSupport.stringValue(requestLog.get("modelCallId"))
        ));
        entity.setOwnerUserId(TaskMybatisValueSupport.longObjectValue(requestLog.get("ownerUserId")));
        entity.setOwnerRefId(ownerRefId);
        entity.setTaskId(taskId);
        entity.setWorkflowId(workflowId);
        entity.setRequestType(firstNonBlank(
            TaskMybatisValueSupport.stringValue(requestLog.get("requestType")),
            TaskMybatisValueSupport.stringValue(requestLog.get("callKind"))
        ));
        entity.setStage(TaskMybatisValueSupport.stringValue(requestLog.get("stage")));
        entity.setOperation(TaskMybatisValueSupport.stringValue(requestLog.get("operation")));
        entity.setProvider(TaskMybatisValueSupport.stringValue(requestLog.get("provider")));
        entity.setProviderModel(TaskMybatisValueSupport.stringValue(requestLog.get("providerModel")));
        entity.setRequestedModel(TaskMybatisValueSupport.stringValue(requestLog.get("requestedModel")));
        entity.setResolvedModel(TaskMybatisValueSupport.stringValue(requestLog.get("resolvedModel")));
        entity.setEndpointHost(TaskMybatisValueSupport.stringValue(requestLog.get("endpointHost")));
        entity.setRequestId(TaskMybatisValueSupport.stringValue(requestLog.get("requestId")));
        entity.setSuccess(success ? 1 : 0);
        entity.setStatus(firstNonBlank(
            TaskMybatisValueSupport.stringValue(requestLog.get("status")),
            success ? "success" : "failed"
        ));
        entity.setHttpStatus(TaskMybatisValueSupport.intValue(requestLog.get("httpStatus"), 0));
        entity.setErrorCode(TaskMybatisValueSupport.stringValue(requestLog.get("errorCode")));
        entity.setErrorMessage(TaskMybatisValueSupport.stringValue(requestLog.get("errorMessage")));
        entity.setStartedAt(TaskMybatisValueSupport.offsetValue(requestLog.get("startedAt")));
        entity.setFinishedAt(TaskMybatisValueSupport.offsetValue(requestLog.get("finishedAt")));
        entity.setDurationMs(TaskMybatisValueSupport.intValue(requestLog.get("durationMs"), 0));
        entity.setTimezoneOffsetMinutes(DEFAULT_TIMEZONE_OFFSET_MINUTES);
        entity.setIsDeleted(0);
        return entity;
    }
    ...
    MaterialAssetEntity toMaterialAssetEntity(String taskId, Map<String, Object> material) {
        ...
        entity.setOwnerUserId(TaskMybatisValueSupport.longObjectValue(material.get("ownerUserId")));
        entity.setTaskId(taskId);
        ...
        entity.setThumbnailUrl(TaskMybatisValueSupport.stringValue(material.get("thumbnailUrl")));
