...
final class TaskWorkerRenderStageService {
    ...
    RenderStageResult render(TaskRecord task, TaskWorkerExecutionContext runContext, RenderStageRequest request) {
        ...
        for (int index = Math.max(0, request.renderStartIndex() - 1); index < request.shotPlans().size(); index++) {
            ...
            Map<String, Object> pendingVideoModelCall = statusStageService.createPendingModelCall(
                task,
            ...
            executionCoordinator.recordModelCall(task, pendingVideoModelCall);
            Map<String, Object> videoRun;
            try {
                videoRun = generationApplicationService.createRun(videoRequest);
            } catch (Exception ex) {
                executionCoordinator.recordModelCall(task, statusStageService.failModelCall(pendingVideoModelCall, ex));
                throw ex;
            }
            ...
            taskRepository.save(task);
            Map<String, Object> videoModelCall = statusStageService.completeModelCall(pendingVideoModelCall, videoRun, videoResult);
    ...
    private FrameResolution generateFrame(
        ...
        Map<String, Object> pendingImageModelCall = statusStageService.createPendingModelCall(
            task,
            TaskStage.PLANNING.code(),
            "generation.image",
            imageRequest,
        ...
        executionCoordinator.recordModelCall(task, pendingImageModelCall);
        Map<String, Object> imageRun;
        try {
            imageRun = generationApplicationService.createRun(imageRequest);
        } catch (Exception ex) {
            executionCoordinator.recordModelCall(task, statusStageService.failModelCall(pendingImageModelCall, ex));
            throw ex;
        }
        runtimeSupport.assertTaskStillActive(task);
        ...
        Map<String, Object> imageModelCall = statusStageService.completeModelCall(pendingImageModelCall, imageRun, imageResult);
