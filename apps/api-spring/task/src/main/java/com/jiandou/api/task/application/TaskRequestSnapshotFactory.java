...
public class TaskRequestSnapshotFactory {
    ...
    public GenerationRequestSnapshot create(CreateGenerationTaskRequest request, TaskRecord task) {
        String taskType = normalizedTaskType(task == null ? "" : task.taskType(), request.taskType());
        return new GenerationRequestSnapshot(
            taskType,
            normalizedAssetType(request.assetType(), taskType),
            task.title(),
            task.creativePrompt(),
            task.aspectRatio(),
            trimmed(request.imageSize(), ""),
            firstNonBlank(
    ...
    private String normalizedTaskType(String taskValue, String requestValue) {
        String normalized = firstNonBlank(requestValue, taskValue, "generation");
        if ((requestValue == null || requestValue.isBlank()) && "video_generation".equals(normalized)) {
            return "generation";
        }
