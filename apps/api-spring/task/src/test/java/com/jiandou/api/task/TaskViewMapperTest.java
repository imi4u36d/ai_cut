...
class TaskViewMapperTest {
    ...
    @Test
    void toListItemCreatesThumbnailFromImageUrl() {
        TaskRecord task = new TaskRecord();
        task.id = "task_image";
        task.title = "demo";
        task.status = "COMPLETED";
        task.executionContext.put("latestImageOutputUrl", "/storage/gen/images/source.png");
        LocalMediaArtifactService mediaService = mock(LocalMediaArtifactService.class);
        when(mediaService.ensureMediaThumbnail("image", "/storage/gen/images/source.png", List.of(), 480))
            .thenReturn("/storage/thumbs/gen/images/source-w480.jpg");

        TaskViewMapper mapper = new TaskViewMapper(storageProperties("../../storage"), mediaService);
        Map<String, Object> row = mapper.toListItem(task);

        assertEquals("/storage/thumbs/gen/images/source-w480.jpg", row.get("thumbnailUrl"));
        verify(mediaService).ensureMediaThumbnail("image", "/storage/gen/images/source.png", List.of(), 480);
    }

    @Test
    void toListItemUsesVideoFirstFrameForThumbnailInsteadOfVideoFile() {
        TaskRecord task = new TaskRecord();
        task.id = "task_video";
        task.title = "demo";
        task.status = "COMPLETED";
        task.executionContext.put("videoOutputUrl", "/storage/gen/videos/clip.mp4");
        task.outputs.add(Map.of(
            "resultType", "video",
            "clipIndex", 1,
            "previewUrl", "/storage/gen/videos/clip.mp4",
            "downloadUrl", "/storage/gen/videos/clip.mp4",
            "extra", Map.of(
                "thumbnailUrl", "/storage/gen/images/clip-first.png"
            )
        ));
        LocalMediaArtifactService mediaService = mock(LocalMediaArtifactService.class);
        when(mediaService.ensureMediaThumbnail("video", "/storage/gen/videos/clip.mp4", List.of("/storage/gen/images/clip-first.png"), 480))
            .thenReturn("/storage/thumbs/gen/images/clip-first-w480.jpg");

        TaskViewMapper mapper = new TaskViewMapper(storageProperties("../../storage"), mediaService);
        Map<String, Object> row = mapper.toListItem(task);

        assertEquals("/storage/thumbs/gen/images/clip-first-w480.jpg", row.get("thumbnailUrl"));
        verify(mediaService).ensureMediaThumbnail("video", "/storage/gen/videos/clip.mp4", List.of("/storage/gen/images/clip-first.png"), 480);
        verify(mediaService, never()).ensureImageThumbnail("/storage/gen/videos/clip.mp4", 480);
    }

    @Test
    void toListItemCreatesThumbnailFromVideoFileWhenCoverIsMissing() {
        TaskRecord task = new TaskRecord();
        task.id = "task_video_no_cover";
        task.title = "demo";
        task.status = "COMPLETED";
        task.executionContext.put("videoOutputUrl", "/storage/gen/videos/clip.mp4");
        task.outputs.add(Map.of(
        ...
        LocalMediaArtifactService mediaService = mock(LocalMediaArtifactService.class);
        when(mediaService.ensureMediaThumbnail("video", "/storage/gen/videos/clip.mp4", List.of(""), 480))
            .thenReturn("/storage/thumbs/gen/videos/clip-video-w480.jpg");

        TaskViewMapper mapper = new TaskViewMapper(storageProperties("../../storage"), mediaService);
        Map<String, Object> row = mapper.toListItem(task);

        assertEquals("/storage/thumbs/gen/videos/clip-video-w480.jpg", row.get("thumbnailUrl"));
        verify(mediaService).ensureMediaThumbnail("video", "/storage/gen/videos/clip.mp4", List.of(""), 480);
    }
